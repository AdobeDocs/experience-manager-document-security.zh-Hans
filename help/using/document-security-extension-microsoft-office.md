---
title: AEM Document Security Extension for Microsoft&reg； Office简介
description: 使用Document Security Extension for Microsoft&reg； Office，您可以将预定义的机密性设置应用于Microsoft&reg； Office文件。
...
* 在管理员或者应用策略的人撤销对文件的访问权限或者更改策略之前。
  ...
  如果应用策略的人更改策略或撤销对文件的访问权限，则即使该用户已拥有该文件，仍将更改或删除该用户对该文件的权限。 如果文件本身被撤销，则会向用户提供URL以获取更新副本。
